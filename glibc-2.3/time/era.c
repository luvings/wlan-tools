...
struct era_entry *
internal_function _nl_get_era_entry(const struct tm *tp, struct locale_data *current)
{
...
struct era_entry *
internal_function _nl_select_era_entry(int cnt, struct locale_data *current)
{
	if (current->private.time == NULL || !current->private.time->era_initialized)
		_nl_init_era_entries(current);
