...
static int32_t *xdrrec_inline(XDR *xdrs, int len)
{
	RECSTREAM *rstrm = (RECSTREAM *) xdrs->x_private;
	int32_t *buf = NULL;
...
internal_function get_input_bytes(RECSTREAM *rstrm, caddr_t addr, int len)
{
	int current;
...
internal_function set_input_fragment(RECSTREAM *rstrm)
{
	uint32_t header;
...
internal_function skip_input_bytes(RECSTREAM *rstrm, long cnt)
{
	int current;
...
static u_int
internal_function fix_buf_size(u_int s)
{
	if (s < 100)
		s = 4000;
