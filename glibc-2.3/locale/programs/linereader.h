...
static inline int lr_getc(struct linereader *lr)
{
	if (lr->idx == lr->bufact) {
		if (lr->bufact != 0)
			if (lr_next(lr) < 0)
...
static inline int lr_ungetc(struct linereader *lr, int ch)
{
	if (lr->idx == 0)
		return -1;
...
static inline void lr_ignore_rest(struct linereader *lr, int verbose)
{
	if (verbose) {
