...
static unsigned int to_upper(unsigned int ch)
{
...
static unsigned int to_lower(unsigned int ch)
{
	if (unicode_attributes[ch].name != NULL
...
static unsigned int to_title(unsigned int ch)
{
	if (unicode_attributes[ch].name != NULL
...
static bool is_upper(unsigned int ch)
{
	return (to_lower(ch) != ch);
}

static bool is_lower(unsigned int ch)
{
...
static bool is_alpha(unsigned int ch)
{
...
static bool is_digit(unsigned int ch)
{
#if 0
	return (unicode_attributes[ch].name != NULL
	        && unicode_attributes[ch].category[0] == 'N'
...
static bool is_outdigit(unsigned int ch)
{
	return (ch >= 0x0030 && ch <= 0x0039);
}

static bool is_blank(unsigned int ch)
{
	return (ch == 0x0009 /* '\t' */
...
static bool is_space(unsigned int ch)
{
...
static bool is_cntrl(unsigned int ch)
{
	return (unicode_attributes[ch].name != NULL
...
static bool is_xdigit(unsigned int ch)
{
#if 0
	return is_digit(ch)
	       || (ch >= 0x0041 && ch <= 0x0046)
...
static bool is_graph(unsigned int ch)
{
	return (unicode_attributes[ch].name != NULL
	        && strcmp(unicode_attributes[ch].name, "<control>")
	        && !is_space(ch));
...
static bool is_print(unsigned int ch)
{
	return (unicode_attributes[ch].name != NULL
	        && strcmp(unicode_attributes[ch].name, "<control>")
	        /* Categories Zl and Zp */
...
static bool is_punct(unsigned int ch)
{
#if 0
	return (unicode_attributes[ch].name != NULL
...
static bool is_combining(unsigned int ch)
{
...
static bool is_combining_level3(unsigned int ch)
{
...
static const char *ucs_symbol(unsigned int i)
{
...
static const char *ucs_symbol_range(unsigned int low, unsigned int high)
{
...
static void output_charclass(FILE *stream, const char *classname,
...
static void output_charmap(FILE *stream, const char *mapname,
...
static void output_widthmap(FILE *stream)
{
}

/* Output the tables to the given file.  */

static void output_tables(const char *filename, const char *version)
{
	FILE *stream;
	unsigned int ch;
