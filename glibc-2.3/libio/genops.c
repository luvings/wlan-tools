...
_IO_ssize_t _IO_least_marker(fp, end_p)
_IO_FILE *fp;
char *end_p;
{
...
void _IO_switch_to_main_get_area(fp)
_IO_FILE *fp;
{
	char *tmp;
...
void _IO_switch_to_backup_area(fp)
_IO_FILE *fp;
{
	char *tmp;
	fp->_flags |= _IO_IN_BACKUP;
...
_IO_FILE *_IO_default_setbuf(fp, p, len)
_IO_FILE *fp;
char *p;
_IO_ssize_t len;
{
...
_IO_off64_t _IO_default_seekpos(fp, pos, mode)
_IO_FILE *fp;
_IO_off64_t pos;
int mode;
{
...
void _IO_default_finish(fp, dummy)
_IO_FILE *fp;
int dummy;
{
	struct _IO_marker *mark;
...
unsigned _IO_adjust_column(start, line, count)
unsigned start;
const char *line;
int count;
{
...
static void _IO_unbuffer_write()
{
	struct _IO_FILE *fp;
...
int _IO_marker_difference(mark1, mark2)
...
int _IO_default_pbackfail(fp, c)
_IO_FILE *fp;
int c;
{
...
_IO_ssize_t _IO_default_read(fp, data, n)
_IO_FILE *fp;
void *data;
_IO_ssize_t n;
{
...
_IO_ssize_t _IO_default_write(fp, data, n)
_IO_FILE *fp;
const void *data;
_IO_ssize_t n;
{
...
int _IO_default_showmanyc(fp)
_IO_FILE *fp;
{
	return -1;
}

void _IO_default_imbue(fp, locale)
_IO_FILE *fp;
void *locale;
{
}
...
_IO_ITER _IO_iter_end()
{
	return NULL;
}

_IO_ITER _IO_iter_next(iter)
_IO_ITER iter;
{
	return iter->_chain;
}

_IO_FILE *_IO_iter_file(iter)
_IO_ITER iter;
{
	return iter;
}
