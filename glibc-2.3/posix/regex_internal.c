...
static reg_errcode_t re_string_allocate(pstr, str, len, init_len, trans, icase)
re_string_t *pstr;
const char *str;
...
static reg_errcode_t re_string_construct(pstr, str, len, trans, icase)
re_string_t *pstr;
const char *str;
int len, icase;
RE_TRANSLATE_TYPE trans;
...
static reg_errcode_t re_string_realloc_buffers(pstr, new_buf_len)
re_string_t *pstr;
int new_buf_len;
{
#ifdef RE_ENABLE_I18N
...
static void re_string_construct_common(str, len, pstr, trans, icase)
const char *str;
int len;
re_string_t *pstr;
RE_TRANSLATE_TYPE trans;
...
static void build_wcs_buffer(pstr)
re_string_t *pstr;
{
	mbstate_t prev_st;
...
static void build_wcs_upper_buffer(pstr)
re_string_t *pstr;
{
	mbstate_t prev_st;
	int byte_idx, end_idx, mbclen, remain_len;
...
static int re_string_skip_chars(pstr, new_raw_idx)
re_string_t *pstr;
int new_raw_idx;
{
	mbstate_t prev_st;
...
static void re_string_translate_buffer(pstr)
re_string_t *pstr;
{
	int buf_idx, end_idx;
	end_idx = (pstr->bufs_len > pstr->len) ? pstr->len : pstr->bufs_len;
...
static reg_errcode_t re_string_reconstruct(pstr, idx, eflags, newline)
re_string_t *pstr;
...
static void re_string_destruct(pstr)
re_string_t *pstr;
{
#ifdef RE_ENABLE_I18N
	re_free(pstr->wcs);
...
static unsigned int re_string_context_at(input, idx, eflags, newline_anchor)
...
static reg_errcode_t re_node_set_alloc(set, size)
re_node_set *set;
int size;
{
	set->alloc = size;
...
static reg_errcode_t re_node_set_init_1(set, elem)
re_node_set *set;
int elem;
{
	set->alloc = 1;
...
static reg_errcode_t re_node_set_init_2(set, elem1, elem2)
re_node_set *set;
int elem1, elem2;
{
	set->alloc = 2;
...
static reg_errcode_t re_node_set_init_copy(dest, src)
re_node_set *dest;
...
static reg_errcode_t re_node_set_add_intersect(dest, src1, src2)
re_node_set *dest;
...
static reg_errcode_t re_node_set_init_union(dest, src1, src2)
re_node_set *dest;
const re_node_set *src1, *src2;
{
	int i1, i2, id;
...
static reg_errcode_t re_node_set_merge(dest, src)
re_node_set *dest;
const re_node_set *src;
{
	int si, di;
...
static int re_node_set_insert(set, elem)
re_node_set *set;
int elem;
{
	int idx, right, mid;
...
static int re_node_set_compare(set1, set2)
...
static int re_node_set_contains(set, elem)
...
static void re_node_set_remove_at(set, idx)
re_node_set *set;
int idx;
{
...
static int re_dfa_add_node(dfa, token, mode)
re_dfa_t *dfa;
re_token_t token;
int mode;
{
...
static unsigned int inline calc_state_hash(nodes, context)
const re_node_set *nodes;
unsigned int context;
{
...
static re_dfastate_t *re_acquire_state(err, dfa, nodes)
reg_errcode_t *err;
re_dfa_t *dfa;
const re_node_set *nodes;
{
...
static re_dfastate_t *re_acquire_state_context(err, dfa, nodes, context)
reg_errcode_t *err;
re_dfa_t *dfa;
const re_node_set *nodes;
unsigned int context;
...
static re_dfastate_t *create_newstate_common(dfa, nodes, hash)
re_dfa_t *dfa;
const re_node_set *nodes;
unsigned int hash;
{
...
static reg_errcode_t register_state(dfa, newstate, hash)
re_dfa_t *dfa;
re_dfastate_t *newstate;
unsigned int hash;
{
...
static re_dfastate_t *create_ci_newstate(dfa, nodes, hash)
re_dfa_t *dfa;
const re_node_set *nodes;
unsigned int hash;
{
...
static re_dfastate_t *create_cd_newstate(dfa, nodes, context, hash)
re_dfa_t *dfa;
const re_node_set *nodes;
