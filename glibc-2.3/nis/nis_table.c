...
static struct ib_request *__create_ib_request(const_nis_name name, unsigned int flags)
{
...
static char *__get_tablepath(char *name, dir_binding *bptr)
{
...
nis_result *nis_add_entry(const_nis_name name, const nis_object *obj2, unsigned int flags)
{
	nis_object obj;
	nis_result *res;
	nis_error status;
