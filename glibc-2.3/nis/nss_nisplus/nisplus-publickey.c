...
enum nss_status _nss_nisplus_getsecretkey(const char *netname, char *skey, char *passwd,
                int *errnop)
{
	nis_result *res;
	enum nss_status retval;
...
static enum nss_status parse_grp_str(const char *s, gid_t *gidp, int *gidlenp, gid_t *gidlist,
                                     int *errnop)
{
	char *ep;
	int gidlen;
...
enum nss_status _nss_nisplus_netname2user(char netname[MAXNETNAMELEN + 1], uid_t *uidp,
