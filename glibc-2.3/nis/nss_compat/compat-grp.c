...
static enum nss_status _nss_first_init(void)
{
	if (ni == NULL) {
...
static enum nss_status getgrent_next_nis(struct group *result, ent_t *ent, char *buffer,
...
static enum nss_status getgrent_next_nisplus(struct group *result, ent_t *ent, char *buffer,
                size_t buflen, int *errnop)
{
	int parse_res;
...
static enum nss_status getgrnam_plusgroup(const char *name, struct group *result, char *buffer,
                size_t buflen, int *errnop)
{
	struct parser_data *data = (void *) buffer;
	int parse_res;
...
static enum nss_status internal_getgrent_r(struct group *gr, ent_t *ent, char *buffer,
                size_t buflen, int *errnop)
{
	if (ent->nis) {
		if (use_nisplus)
...
static enum nss_status getgrgid_plusgroup(gid_t gid, struct group *result, char *buffer,
                size_t buflen, int *errnop)
{
	struct parser_data *data = (void *) buffer;
	int parse_res;
