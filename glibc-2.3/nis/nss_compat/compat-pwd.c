...
static enum nss_status insert_passwd_adjunct(char **result, int *len, char *domain, int *errnop)
{
...
static enum nss_status internal_setpwent(ent_t *ent)
{
...
static enum nss_status getpwent_next_nis_netgr(const char *name, struct passwd *result, ent_t *ent,
...
static enum nss_status getpwent_next_nisplus_netgr(const char *name, struct passwd *result,
...
static enum nss_status getpwent_next_nisplus(struct passwd *result, ent_t *ent, char *buffer,
                size_t buflen, int *errnop)
{
	int parse_res;
	size_t p2len;
...
static enum nss_status getpwent_next_nis(struct passwd *result, ent_t *ent, char *buffer,
                size_t buflen, int *errnop)
{
	struct parser_data *data = (void *) buffer;
...
static bool_t in_blacklist(const char *name, int namelen, ent_t *ent)
{
	char buf[namelen + 3];
	char *cp;
