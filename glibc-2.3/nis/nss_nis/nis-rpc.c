...
static int saveit(int instatus, char *inkey, int inkeylen, char *inval,
...
static enum nss_status internal_nis_setrpcent(intern_t *intern)
{
	char *domainname;
...
enum nss_status _nss_nis_setrpcent(int stayopen)
{
	enum nss_status status;
...
static enum nss_status internal_nis_endrpcent(intern_t *intern)
{
	while (intern->start != NULL) {
		if (intern->start->val != NULL)
			free(intern->start->val);
...
enum nss_status _nss_nis_endrpcent(void)
{
	enum nss_status status;
...
static enum nss_status internal_nis_getrpcent_r(struct rpcent *rpc, char *buffer, size_t buflen,
...
enum nss_status _nss_nis_getrpcent_r(struct rpcent *rpc, char *buffer, size_t buflen,
                                     int *errnop)
{
	enum nss_status status;
...
enum nss_status _nss_nis_getrpcbyname_r(const char *name, struct rpcent *rpc,
...
enum nss_status _nss_nis_getrpcbynumber_r(int number, struct rpcent *rpc,
                char *buffer, size_t buflen, int *errnop)
{
