...
static enum nss_status internal_nis_setservent(intern_t *intern)
{
	char *domainname;
...
static enum nss_status internal_nis_endservent(intern_t * intern)
{
	while (intern->start != NULL) {
		if (intern->start->val != NULL)
			free(intern->start->val);
...
static enum nss_status internal_nis_getservent_r(struct servent *serv, char *buffer,
...
enum nss_status _nss_nis_getservent_r(struct servent *serv, char *buffer, size_t buflen,
                                      int *errnop)
{
	enum nss_status status;
...
enum nss_status _nss_nis_getservbyname_r(const char *name, const char *protocol,
...
enum nss_status _nss_nis_getservbyport_r(int port, const char *protocol,
