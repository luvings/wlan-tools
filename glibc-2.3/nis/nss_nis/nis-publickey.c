...
enum nss_status _nss_nis_getsecretkey(const char *netname, char *skey, char *passwd,
                                      int *errnop)
{
	enum nss_status retval;
...
static enum nss_status parse_netid_str(const char *s, uid_t *uidp, gid_t *gidp, int *gidlenp,
                                       gid_t *gidlist)
{
	char *p, *ep;
	int gidlen;
...
enum nss_status _nss_nis_netname2user(char netname[MAXNETNAMELEN + 1], uid_t *uidp,
