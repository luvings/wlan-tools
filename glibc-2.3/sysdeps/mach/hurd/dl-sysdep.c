...
void
internal_function _dl_sysdep_start_cleanup(void)
{
...
/* This is called only in some strange cases trying to guess a value
   ...
   `open', above.)  */
char *
weak_function __getcwd(char *buf, size_t size)
{
	errno = ENOSYS;
	return NULL;
}

void weak_function _exit(int status)
{
...
void
internal_function _dl_show_auxv(void)
{
...
internal_function _dl_important_hwcaps(const char *platform, size_t platform_len, size_t *sz,
