...
static void cache_addgr(struct database *db, int fd, request_header *req, void *key,
...
void addgrbyname(struct database *db, int fd, request_header *req,
                 void *key, uid_t uid)
{
...
void addgrbygid(struct database *db, int fd, request_header *req,
                void *key, uid_t uid)
{
	/* Search for the entry matching the key.  Please note that we don't
	   look again in the table whether the dataset is now available.  We
