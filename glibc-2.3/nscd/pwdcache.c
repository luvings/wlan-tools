...
static void cache_addpw(struct database *db, int fd, request_header *req, void *key,
...
void addpwbyname(struct database *db, int fd, request_header *req,
...
void addpwbyuid(struct database *db, int fd, request_header *req,
                void *key, uid_t c_uid)
{
	/* Search for the entry matching the key.  Please note that we don't
	   look again in the table whether the dataset is now available.  We
