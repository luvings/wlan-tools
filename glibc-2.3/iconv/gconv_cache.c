...
int
internal_function __gconv_load_cache(void)
{
	int fd;
	struct stat64 st;
...
static int
internal_function find_module_idx(const char *str, size_t *idxp)
{
	unsigned int idx;
	unsigned int hval;
	unsigned int hval2;
...
#ifndef STATIC_GCONV
static int
internal_function find_module(const char *directory, const char *filename,
...
int
internal_function __gconv_compare_alias_cache(const char *name1, const char *name2, int *result)
{
	size_t name1_idx;
	size_t name2_idx;
...
int
internal_function __gconv_lookup_cache(const char *toset, const char *fromset,
...
void
internal_function __gconv_release_cache(struct __gconv_step *steps, size_t nsteps)
{
	if (gconv_cache != NULL)
