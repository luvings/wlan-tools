...
static enum nss_status internal_setent(void)
{
...
static enum nss_status get_next_alias(const char *match, struct aliasent *result,
...
enum nss_status _nss_files_getaliasbyname_r(const char *name, struct aliasent *result,
                char *buffer, size_t buflen, int *errnop)
{
	/* Return next entry in host file.  */
	enum nss_status status = NSS_STATUS_SUCCESS;
