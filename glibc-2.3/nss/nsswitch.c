...
/* -1 == not found
    ...
    1 == finished */
int __nss_next(service_user **ni, const char *fct_name, void **fctp, int status,
               int all_values)
{
	if (all_values) {
...
internal_function nss_parse_file(const char *fname)
{
	FILE *fp;
...
/* Read the source names:
   ...
   */
static service_user *
internal_function nss_parse_service_list(const char *line)
{
...
internal_function nss_getline(char *line)
{
	const char *name;
...
internal_function nss_new_service(name_database *database, const char *name)
{
