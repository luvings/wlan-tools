...
int iw_get_range_info(int		skfd,
                      char 	*ifname,
                      iwrange *	range);
int iw_print_version_info(char *	toolname);
...
int iw_protocol_compare(char *	protocol1,
                        char 	*protocol2);
/* -------------------- FREQUENCY SUBROUTINES --------------------- */
void iw_float2freq(double	in,
                   iwfreq *	out);
double iw_freq2float(iwfreq *	in);
...
int iw_check_if_addr_type(int	skfd,
                          char 	*ifname);
#if 0
int iw_check_addr_type(int		skfd,
                       char 	*ifname);
#endif
void iw_ether_ntop(const struct ether_addr* eth, char* buf);
char *iw_ether_ntoa(const struct ether_addr* eth);
 ...
 * struct sockaddr... */
static inline char *iw_pr_ether(char* bufp, const unsigned char* addr)
{
...
/* Backwards compatability */
static inline int iw_in_ether(const char *bufp, struct sockaddr *sap)
{
